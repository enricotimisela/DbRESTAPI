...
class PHP_CRUD_API {
	...
	protected function convertFilter($field, $comparator, $value) {
		switch (strtolower($comparator)) {
			...
			case 'ics': $comparator = 'ILIKE'; $value = '%'.$this->db->likeEscape($value).'%'; break;  //case-insensitive
			case 'isw': $comparator = 'ILIKE'; $value = $this->db->likeEscape($value).'%'; break; //case-insensitive
			case 'iew': $comparator = 'ILIKE'; $value = '%'.$this->db->likeEscape($value); break; //case-insensitive
...
// For PostgreSQL 9 use:
 $api = new PHP_CRUD_API(array(
 	'dbengine'=>'PostgreSQL',
 	'hostname'=> $vcap_services->{'postgres'}[0]->credentials->host,
 	'username'=> $vcap_services->{'postgres'}[0]->credentials->username,
 	'password'=> $vcap_services->{'postgres'}[0]->credentials->password,
 	'database'=> $vcap_services->{'postgres'}[0]->credentials->database,
 	'charset'=>'UTF8'
 ));
 $api->executeCommand();
