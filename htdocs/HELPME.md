...
Search is implemented with the "filter" parameter. You need to specify the column name, a comma, the match type, another commma and the value you want to filter on. These are supported match types:
  ...
  - ics: contain string (string contains value case-insensitive)  --by:EnTi for Predix Postgres
  - isw: start with (string starts with value case-insensitive)  --by:EnTi for Predix Postgres
  - iew: end with (string end with value case-insensitive)  --by:EnTi for Predix Postgres
